...
const Help = () => {
  ...
  console.log(portfolio)
     useEffect(() => {
       ...
       const [request, setRequest] = useState({
        ...
        id_medcin: portfolio.id_medcin,
        nom_prenom: portfolio.nom_prenom,
        id_specialite:portfolio.id_specialite,
        genre:'',
        id_partie_corp:'',
        malade: '',
        description_malade:'',
        symptomes: ['', '', ''], // Liste de symptômes initialisée avec trois chaînes vides
        consignes: ['', '', '']  // Liste de consignes initialisée avec trois chaînes vides
      });
  ...
  const afficher=(id)=>{
    setRequest({...request,id_partie_corp:id})
    setAdd(true)
  }
  ...
  const handleChange = (e) => {
    ...
    if (name === 'malade'  ) {
      setRequest(prevState => ({ ...prevState, malade: value }));
    } else if (name.startsWith('symptome')) {
      const index = parseInt(name.replace('symptome', ''), 10) - 1;
      setRequest(prevState => {
        const symptomes = [...prevState.symptomes];
        symptomes[index] = value;
        return { ...prevState, symptomes };
      });
    } else if (name.startsWith('consigne')) {
      const index = parseInt(name.replace('consigne', ''), 10) - 1;
      setRequest(prevState => {
        const consignes = [...prevState.consignes];
        consignes[index] = value;
        return { ...prevState, consignes };
      });
    }else if(name === 'description_malade' ){
      setRequest(prevState => ({ ...prevState, description_malade: value }));
    }else if(name ==='genre')
      {
        setRequest(prevState => ({ ...prevState, genre: value }));
      }
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if(request.id!=='' && request.id_medcin!=='' && request.nom_prenom!=='' && request.id_specialite!=='' && request.genre!=='' && request.id_partie_corp!=='' && request.malade!==''&& request.description_malade!=='' ){
      
    // Faites quelque chose avec request, par exemple, envoyer à un backend
    console.log('hadi hiya:',request);
    axios.post("http://localhost:3001/request",request).then((res)=>{
    ...
    setAdd(false)
         ...
         alert ('tous les champ obligatoire') 
    }
      ...
      {console.log(list_partie)}
        ...
        <div>
          ...
          <h2 className="d">
            ...
            Veuillez choisir la zone ou vous avez ajouter le malade :
          </h2>
  ...
  <div >
    <label htmlFor="genre" className="font-weight-bold">Genre</label>
    <select name="genre" id="" className="form-control" onClick={handleChange}>
  ...
  <div >
    <label htmlFor="malade" className="font-weight-bold">Malade</label>
    <input type="text" name="malade" className="form-control" onChange={handleChange} />
  </div>
  <div >
    <label htmlFor="description_malade" className="font-weight-bold">Description du malade</label>
    <input type="text" name="description_malade" className="form-control" onChange={handleChange} />
  </div>
  <div >
    <label className="font-weight-bold">Liste de symptômes</label>
    <div className="row">
      <div className="col-md-4">
        <input type="text" name="symptome1" className="form-control" onChange={handleChange} />
      </div>
      <div className="col-md-4">
        <input type="text" name="symptome2" className="form-control" onChange={handleChange} />
      </div>
      <div className="col-md-4">
        <input type="text" name="symptome3" className="form-control" onChange={handleChange} />
      </div>
    </div>
  </div>
  <div >
    <label className="font-weight-bold">Liste de consignes</label>
    <div className="row">
      <div className="col-md-4">
        <input type="text" name="consigne1" className="form-control" onChange={handleChange} />
      </div>
      <div className="col-md-4">
        <input type="text" name="consigne2" className="form-control" onChange={handleChange} />
      </div>
      <div className="col-md-4">
        <input type="text" name="consigne3" className="form-control" onChange={handleChange} />
      </div>
    </div>
  </div>
  <button type="submit" 
